...
export class UserRolesService {
  ...
  constructor(@InjectModel(UserRole.name) private UserRoleModel: Model<UserRoleDocument>,private roleService: RolesService,private usersService: UsersService) {}
  ...
  async update(id: string, updateUserRoleDto: UpdateUserRoleDto) {
    try{
      await this.UserRoleModel.updateOne({_id:id},{$set:{...updateUserRoleDto}});
      return await this.UserRoleModel.findOne({_id:id}); 
      }
    catch
      {
