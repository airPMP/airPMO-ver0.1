...
export class ExcelService {
  ...
  async quantityFile(files, req) {
    ...
    if (sheet) {
      var qunatityjsondata = await xlsx.utils.sheet_to_json(sheet, {
        dateNF: 'YYYY-MM-DD',
      });
    } else {
      throw new NotFoundException('file not found');
    ...
    var quantity_find_data = await this.excelModel.findOne({
      project_id: projectid,
    });
    if (!quantity_find_data) {
      ...
      if (files[0].fieldname === 'quantity_sheet' && projectid) {
        var update_quantity = await this.excelModel.updateOne(
          { project_id: projectid },
          { quantity_sheets: new_ar2 },
        );
        return 'update sucessfully';
  ...
  async firequantityFile(files: any, @Req() req) {
    ...
    if (sheet) {
      var fire_json_Data = await xlsx.utils.sheet_to_json(sheet, {
        dateNF: 'YYYY-MM-DD',
      });
    } else {
      throw new NotFoundException('sheet  not found');
    ...
    if (!find_fire_data) {
      ...
      if (files[0].fieldname === 'fire_quantity_sheet' && projectid) {
        var update_fire_quantity = await this.excelModel.updateOne(
          { project_id: projectid },
          { fire_quantity_sheets: fire_all_data },
        );
        return 'update sucessfully';
  ...
  async lightquantityFile(files: any, @Req() req) {
    ...
    if (sheet) {
      var light_fitting_json_Data = await xlsx.utils.sheet_to_json(sheet, {
        dateNF: 'YYYY-MM-DD',
      });
    } else {
      throw new NotFoundException('sheet  not found');
            ...
            if (k < 2) {
              ...
              if (lights_zone_values.length - 1 === index) {
                if (light_fitting_arrays.length != 0) {
                  ...
                  total_zone_value = lodash.sum(
                    light_array_all_value.splice(2),
                  );
                  light_fitting_arrays[k] = Object.assign({
                    total: total_zone_value,
                  });
    ...
    var find_light_data = await this.excelModel.findOne({
      project_id: projectid,
    });
    if (!find_light_data) {
      ...
      if (files[0].fieldname === 'light_quantity_sheet' && projectid) {
        var update_light_quantity = await this.excelModel.updateOne(
          { project_id: projectid },
          { light_fitting_quantity_sheets: light_fitting_all_data },
        );
        return 'update sucessfully';
