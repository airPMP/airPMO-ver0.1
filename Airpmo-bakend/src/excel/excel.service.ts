import { BadRequestException, Body, Injectable, NotFoundException, Req, UnprocessableEntityException } from '@nestjs/common';
...
export class ExcelService {
  ...
  async quantityFile(files, req) {
    ...
    try {
      ...
      if (new_ar2.length != 0) {
        ...
        throw new BadRequestException("plz upload proper format")
      }
    } catch {
      throw new NotFoundException("sheet not found")
    }
  ...
  async lightquantityFile(files: any, @Req() req) {
    ...
    if (light_fitting_all_data.length != 0) {
